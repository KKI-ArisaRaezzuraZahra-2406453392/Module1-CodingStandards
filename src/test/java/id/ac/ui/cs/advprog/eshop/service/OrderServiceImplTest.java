...
class OrderServiceImplTest {
    ...
    void setUp() {
        ...
        Order order1 = new Order("first-order-1",
                products, 1708560000L, "Safira Sudrajat");
        orders.add(order1);
        Order order2 = new Order("second-order-2",
    ...
    void testUpdateStatusInvalidOrderId() {
        ...
        assertThrows(NoSuchElementException.class, () ->
                orderService.updateStatus("invalid-id", OrderStatus.SUCCESS.getValue())) ;

        verify(orderRepository, times(0)).save(any(Order.class));
    }

    @Test
    void testByIdIfIdFound() {
        Order order = orders.get(1);
        doReturn(order).when(orderRepository).findById(order.getId());
    ...
    void testFindByIdIfIdNotFound() {
        doReturn(null).when(orderRepository).findById("random-id");
        assertNull(orderService.findById("random-id"));
    }
